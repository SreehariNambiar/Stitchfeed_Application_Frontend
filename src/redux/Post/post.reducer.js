...
export const postReducer = (state=initialState, action) => {
    switch(action.type) {
        ...
        case GET_ALL_POST_SUCCESS:
            return {...state, loading:false, error:null, posts:action.payload, comments:action.payload.comments}
        case GET_USERS_POST_SUCCESS:
            return {...state, posts:action.payload}
        case LIKE_POST_SUCCESS:
            return {...state, 
                like:action.payload, posts:state.posts.map((item) => item.id === action.payload.id? action.payload:item), 
